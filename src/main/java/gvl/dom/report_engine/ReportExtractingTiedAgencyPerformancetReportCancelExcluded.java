...
public class ReportExtractingTiedAgencyPerformancetReportCancelExcluded {
	final static Logger logger = Logger.getLogger(ReportExtractingTiedAgencyPerformancetReportCancelExcluded.class);
	public static void main(String[] args) {
		Properties systemProperties = Utils.loadSystemProperties("config_report_tiedagency_performance_cancel_excluded.properties");
		...
		tiedAgencyPerformanceSegmentReport.fetchDataForCasecountSegDtlSheetCancelExcluded(excelReport, excelReport, m0Start, m0End);
		/* --------------------------------------------------------------------------------- */
		/* --------------------------------------------------------------------------------- */
		if(logger.isInfoEnabled()){
		...
		tiedAgencyPerformanceSegmentReport.fetchDataForSheet_data_detail_seg_chart_y0_case_cancel_excluded(excelReport, excelReport, y0, m0End);
		...
		tiedAgencyPerformanceSegmentReport.fetchDataForSheet_data_casesize_total_cancel_exclude(excelReport, excelReport, "group_casesize_monthly_12ms_y1",y1, y1End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for group_casesize_monthly sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForSheet_data_casesize_total_cancel_exclude(excelReport, excelReport, "group_casesize_monthly",y0, m0End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for case_per_active_monthly_12ms_y1 sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForSheet_data_caseperactive_total_cancelexcl(excelReport, excelReport, "case_per_active_monthly_12ms_y1",y1, y1End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for case_per_active_monthly sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForSheet_data_caseperactive_total_cancelexcl(excelReport, excelReport, "case_per_active_monthly",y0, m0End);
		...
		tiedAgencyPerformanceSegmentReport.fetchDataForDataCasecountMomSheetCancelExcl(excelReport, excelReport, "data_case_mom", m0Start, m0End, 9);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for data_case_yoy sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForDataCasecountMomSheetCancelExcl(excelReport, excelReport, "data_case_yoy", y0, m0End, 9);
		...
		tiedAgencyPerformanceSegmentReport.fetchDataForDataCasesizeMomSheetCancelExclude(excelReport, excelReport, "data_group_casesize_mom", m0Start, m0End, 9);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for data_group_casesize_yoy sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForDataCasesizeMomSheetCancelExclude(excelReport, excelReport, "data_group_casesize_yoy", y0, m0End, 9);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for data_caseperactive_mom sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForDataCaseperactiveMomSheetCancelExcluded(excelReport, excelReport, "data_caseperactive_mom", m0Start, m0End, 10);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for data_caseperactive_yoy sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForDataCaseperactiveMomSheetCancelExcluded(excelReport, excelReport, "data_caseperactive_yoy", y0, m0End, 10);
		...
		tiedAgencyPerformanceSegmentReport.fetchDataForDataRookie90daysSheetCancelExcluded(excelReport, excelReport, y0, m0End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for data_product_mix_ape sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForProductMixAPESheetCancelExclude(excelReport, excelReport, y0, m0End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for data_product_rider_mix sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForProductMixRiderSheetCancelExcluded(excelReport, excelReport, y0, m0End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for data_product_mix_total sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForProductMixTotalSheetCancelExcluded(excelReport, excelReport, y0, m0End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for data_product_rider_mix_count sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForProductMixCountSheetCancelExcluded(excelReport, excelReport, y0, m0End);
