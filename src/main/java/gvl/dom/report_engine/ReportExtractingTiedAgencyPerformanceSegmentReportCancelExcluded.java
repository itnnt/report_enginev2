...
public class ReportExtractingTiedAgencyPerformanceSegmentReportCancelExcluded {
	final static Logger logger = Logger.getLogger(ReportExtractingTiedAgencyPerformanceSegmentReportCancelExcluded.class);
	public static void main(String[] args) {
		Properties systemProperties = Utils.loadSystemProperties("config_report_tiedagency_kpi_segmentation_cancel_excluded.properties");
...
//		String m0End = "2018-07-31";
		...
		tiedAgencyPerformanceSegmentReport.fetchDataForCountrySheetCancelExcluded(excelReport, excelReport, y0, m0End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for North sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForNorthSheetCancelExcluded(excelReport, excelReport, y0, m0End);
		
		if(logger.isInfoEnabled()){
			logger.info("Fetching data for South sheet");
		}
		tiedAgencyPerformanceSegmentReport.fetchDataForSouthSheetCancelExcluded(excelReport, excelReport, y0, m0End);
